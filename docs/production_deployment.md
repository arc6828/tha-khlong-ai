...
- คำขอเชื่อมต่อ WebSockets (`/ws`) -> ส่งไปที่ Companion Server (พอร์ต `3004`)
...
server {
    ...
    # 2. เพิ่มเติม: จัดการระบบซิงก์เรียลไทม์ WebSockets (พอร์ต 3004)
    location /ws {
        proxy_pass http://127.0.0.1:3004; # ส่งต่อไปที่ WebSocket Companion Server
        proxy_http_version 1.1;
...
2. **Zero Insecure Ports**: ไม่จำเป็นต้องเปิดพอร์ต `3004` ออกสู่อินเทอร์เน็ตภายนอก (ไม่ต้องตั้งค่า UFW firewall เพิ่มเติมสำหรับพอร์ต `3004`) ช่วยยกระดับความปลอดภัยให้กับเซิร์ฟเวอร์โรงเรียน
